...
#define DOLLARS                     0
#define FORMAT_QUALIFICATEUR        4²²²²²²²                                                                                  c<                ///  format des qualificateurs IMPORTANT
...
unsigned int getF=0;
/*Paramètres*/
#define SEUIL_HAUT 275
#define SEUIL_BAS 240
/*autres*/
unsigned long time;
...
void loop()
{
     if(maTrame._statut[STA] == true)
      {
        digitalWrite(LED13,ON);
        maTrame._statut[STA] = false;
      }
    if(maTrame._statut[RES] == true)
     {
         digitalWrite(LED13,OFF);
         maTrame._statut[RES] = false;
     }
    ...
    ///gestion de la valeur de getV ==>0<slide<511
    if ((getV>=0)&&(getV<SEUIL_BAS))
    {
      driverIBT2.reculer(getV,true);
    }

        if ((getV>SEUIL_BAS)&&(getV<SEUIL_HAUT))
    {
      driverIBT2.arreter(true);
    }

    if ((getV>=SEUIL_HAUT)&&(getV<511))
    {
      driverIBT2.avancer(getV,true);
    }
